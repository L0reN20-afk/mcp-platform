...
export function generateBlueColorsMobile(colors: Float32Array, count: number) {
  for (let i = 0; i < count; i++) {
    ...
    colors[i * 3 + 1] = 0.5 + Math.random() * 0.3 // G - Più equilibrato  
    colors[i * 3 + 2] = 0.7 + Math.random() * 0.2 // B - Blu moderato
  }
}

// 🟣 COLORI VIOLA MOBILE (Features Section) - Contrasto equilibrato
...
// 🔷 COLORI CYAN MOBILE (Packages Section) - Luminosità moderata
